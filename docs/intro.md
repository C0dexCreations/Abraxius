...
Roblox MCP Node is a background MCP bridge and CLI for Roblox Studio. It replaces the need for `StudioMCP.exe` by listening on the same WebSocket endpoint Roblox Studio already connects to (`ws://localhost:13469/studio`) and exposing a local HTTP API on `localhost:13470`.
...
- **Sync**: Pulls scripts from Studio into a local project and pushes local edits back with a focused, two-way sync workflow.
